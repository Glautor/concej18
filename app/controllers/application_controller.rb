...
#  before_action :authenticate
  ...
  def user_must_have_changed_ej
    if @user.junior_enterprise == "Nome da sua EJ"
      redirect_to proc { edit_user_registration_path(@nome_da_ej) }, alert: "Cadastre o nome da sua EJ conforme o aviso na tela inicial especifica"
    end
  end
  ...
  def event_open?
    day_open = DateTime.new(2018, 04, 27, 18, 0, 0 , '-3')
    now = DateTime.now
  ...
  def maintenance_mode_enabled?
    ...
    dia20 = DateTime.new(2017, 8, 23, 21, 59, 50 , '-3')
    now = DateTime.now
